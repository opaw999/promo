<?php	
	...
	$dateToday = date("F d, Y"); //date of today
	...
	if (!empty($store)) {
		...
		$bunit = explode("/", $store);
		$where .= " AND $bunit[1]= 'T'";
		$condition = "WHERE bunit_field = '$bunit[1]'";
	} 
	...
	$query = mysql_query("SELECT employee3.emp_id, position, promo_company, promo_department, eocdate
							FROM employee3 INNER JOIN promo_record ON employee3.emp_id=promo_record.emp_id WHERE (emp_type = 'Promo' OR emp_type = 'Promo-NESCO' OR emp_type = 'Promo-EasyL') AND current_status = 'Active' $where ORDER BY eocdate, name ASC")or die(mysql_error());
