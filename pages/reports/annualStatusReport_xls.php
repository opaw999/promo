...
/********************************** values being passed from report_details **************************/
	...
	$where = " AND (startdate BETWEEN '$dateFrom' AND '$dateTo')";
...
<html>
	...
	<body>
		...
		<center><h4>Annual Status Report from <?php echo date("F d, Y", strtotime($dateFrom)); ?> to <?php echo date("F d, Y", strtotime($dateTo)); ?></h4></center>
			...
			<?php 
				...
				$query = mysql_query("SELECT employee3.emp_id, name, current_status, startdate, eocdate, position, promo_company, promo_department, promo_type, company_duration
											FROM employee3, promo_record 
												WHERE employee3.emp_id = promo_record.emp_id AND (emp_type = 'Promo' OR emp_type = 'Promo-NESCO' OR emp_type = 'Promo-EasyL') $where ORDER BY eocdate, name ASC")or die(mysql_error());
...
//********************************* for report logs	*********************************************//
	...
	$activity 		= "Generate Annual Status Report from ".date("F d, Y", strtotime($dateFrom))." to ".date("F d, Y", strtotime($dateTo));
	$date 			= date("Y-m-d");
	$time 			= date("H:i:s");
